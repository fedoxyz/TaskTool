...
app.delete("/api/taskboards/:id", router, async (req, res, next) => {
  console.log("we here");
  try {
    const taskboard = await db.TaskBoard.findOne({
  ...
  try {
    ...
    if (req.body.assigneeName) {
      try {
        ...
        const assigneeUser = await db.User.findOne({
          where: { username: req.body.assigneeName },
        });
    ...
    const task = await db.Task.create({
      taskboard_id: req.body.taskboard_id,
      title: req.body.title,
      description: req.body.description,
      due_date: req.body.due_date.value,
      assignee_id: assigneeId,
      assignee_name: req.body.assigneeName,
...
app.post("/api/update-task", router, async (req, res, next) => {
  ...
  try {
    ...
    console.log(req.body.assignee_name, "ASSIGNEE NAME");
    // IF ASSIGNEE NAME, CHECK TRY TO FIND IT IN DB
    if (req.body.assignee_name) {
      console.log(req.body.assignee_name, "ASSIGNEE NAME");
      const assigneeUser = await db.User.findOne({
        where: { username: req.body.assignee_name },
      });
      console.log(assigneeUser, "assinged user found");
      const taskAssignment = await db.TaskAssignment.findOne({
        where: { id: task.id },
      });
      if (!taskAssignment) {
        await db.TaskAssignment.create({
      ...
      console.log(taskAssignment, "task assignment");

      taskUpdated = await task.update({
        title: req.body.title,
        description: req.body.description,
