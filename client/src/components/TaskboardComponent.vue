    ...
    <div class="row align-items-start">
      <div v-if="isTasksToggled" class="tasks-tab">
        ...
        <div class="d-flex">
            ...
            >
              <template #item="{ element: task }">
                <li id="task">
                  <div
                    tag="div"
                    class="task d-flex align-self-left"
                    :class="{ selected: selectedTask.value.id == task.id }"
                  >
                    <div class="task-wrapper" @mousedown="taskSelect(task)">
                      {{ task.title }}
                    </div>
                  </div>
                </li>
          ...
          >
            ...
            <div
              v-if="data.isMessage.value"
              :class="{ error: data.isError.value, message: true }"
            >
              {{ data.message.value }}
            </div>
            <draggable
        ...
        <div class="controls">
          <button class="main" id="button" @click="taskToggle()">
            Add Task
          </button>
          <router-link to="/app">
      ...
      <div v-if="!isTasksToggled" class="add-tab">
        ...
        <input
          ...
          id="input"
          v-model="task.assigneeName.value"
        />
  ...
  socket.on("task-updated", async (taskUpdated, edited) => {
    ...
    if (taskUpdated.status === 0) {
      ...
      if (taskIndex !== -1) {
        ...
        taskIndex = progress.value.findIndex(
        ...
        if (taskIndex !== -1) {
          ...
          taskIndex = completed.value.findIndex(
            (t) => t.id === Number(taskUpdated.id)
          );
          if (taskIndex !== -1) {
            ...
            taskIndex = todo.value.findIndex(
              (t) => t.id === Number(taskUpdated.id)
            );
...
async function addTask() {
  try {
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({
        taskboard_id: task.taskboardId.value,
        title: task.title.value,
        description: task.description.value,
        due_date: task.dueDate,
        assigneeName: task.assigneeName.value,
      }),
    };
